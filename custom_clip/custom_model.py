...
from mypy.test.teststubgen import self_arg
...
class MLPAligner(nn.Module):
    ...
    def forward(self, x):
        ...
        return output
class ResidualAttentionBlock_CMP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        if self.add_prompt:
            if self.text_layer:
                ctx_vectors = torch.empty(self.n_ctx_text, d_model)
                self.scale_vector_image = nn.Parameter(torch.randn(512) * 0.02)
            elif not self.text_layer:
                ctx_vectors = torch.empty(self.n_ctx_visual, d_model)
                self.scale_vector = nn.Parameter(torch.randn(768) * 0.02)
            nn.init.normal_(ctx_vectors, std=0.02)
            self.VPT_shallow = nn.Parameter(ctx_vectors)
        else:
            self.add_prompt = False

    def attention(self, x: torch.Tensor, cross_kv: torch.Tensor = None):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        if cross_kv is not None:
            cross_attn_output = self.attn(x, x, cross_kv, need_weights=False, attn_mask=self.attn_mask)[0]
            attn_output = cross_attn_output
        else:
            attn_output = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        return attn_output

    def forward(self, x: torch.Tensor, cross_VPT=None):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                ...
                prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                visual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                visual_context = visual_context + (self.scale_vector * cross_VPT).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                x = torch.cat([prefix, visual_context], dim=0)
            else:
                # [77,n_cls,512]
                prefix = x[:1, :, :]
                suffix = x[1 + self.n_ctx_text:, :, :]
                textual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                textual_context = textual_context + (self.scale_vector_image * cross_VPT).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                x = torch.cat([prefix, textual_context, suffix], dim=0)
            x = x + self.attention(self.ln_1(x))
        else:
            x = x + self.attention(self.ln_1(x))
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'MPrompt':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer in {"DCPS", "CPrompt"}:
            self.resblocks = nn.ModuleList([ResidualAttentionBlock_CMP(width, heads, attn_mask, True,
                                                                       text_layer, i,
                                                                       design_details) if prompts_needed > i
                                            else ResidualAttentionBlock_CMP(width, heads, attn_mask, False,
                                                                            text_layer, i, design_details)
                                            for i in range(layers)])


        else:
            self.resblocks = nn.ModuleList([ResidualAttentionBlock_IVLP(width, heads, attn_mask, True,
                                                                        text_layer, i,
                                                                        design_details) if prompts_needed > i and i != 0
                                            else ResidualAttentionBlock_IVLP(width, heads, attn_mask, False,
                                                                             text_layer, i, design_details)
                                            for i in range(layers)])

    def forward(self, x: torch.Tensor, VPTs: torch.Tensor = None):
        if VPTs is not None:
            for i, resblock in enumerate(self.resblocks):
                if resblock.add_prompt:
                    x = resblock(x, VPTs[i])
        ...
        return x
class Transformer_origin(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
                 text_layer=False, design_details=None):
        super().__init__()
        self.width = width
        ...
        self.resblocks = nn.Sequential(
            *[ResidualAttentionBlock_IVLP(width, heads, attn_mask, False,
                                          text_layer, i, design_details)
              for i in range(layers)])
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            elif trainer == "HPrompt":
                self.visual = VisualTransformer_HPrompt(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
                    ...
                    output_dim=visual_embed_dim,
                    design_details=design_details)
            elif trainer in {"DCPS", "CPrompt"}:
                self.visual = VisualTransformer_CPrompt(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
        ...
        self.transformer_origin = Transformer_origin(
            width=transformer_width,
            layers=transformer_layers,
            heads=transformer_heads,
            attn_mask=self.build_attention_mask(),
        ...
        self.text_projection = nn.Parameter(torch.empty(transformer_width, visual_embed_dim))
    ...
    def encode_image(self, image, cross_kv=None):
        if cross_kv:
                return self.visual(image.type(self.dtype), cross_kv)
        return self.visual(image.type(self.dtype))

    def encode_text(self, text, cross_kv=None):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        if cross_kv:
            x, _ = self.transformer(x, cross_kv)
        else:
...
def build_model(state_dict: dict, design_details):
    ...
    visual_embed_dim = state_dict["text_projection"].shape[1]
    ...
    for p in model.parameters():
        p.data = p.data.float()
    return model.eval()
class ResidualAttentionBlock_Hprompt(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x + self.mlp(self.ln_2(x))
        return x
class VisualTransformer_HPrompt(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, shallow_visual_prompts):
        ...
        if self.VPT_shallow:
            # visual_ctx = self.VPT.expand(x.shape[0], -1, -1).half()
            x = torch.cat([x, shallow_visual_prompts], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        return x
class VisualTransformer_CPrompt(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, VPTs=None):
        # x -> image
        x = self.conv1(x)
